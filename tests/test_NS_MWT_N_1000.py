...
def load_data():
    ...
    data_path = 'Data/NS/ns_V1e-3_N5000_T50.mat'
    ntrain = 1000
    ntest = 200
    ...
    return x_train, y_train, x_test, test_u, y_normalizer

 

def main():
    x_train, y_train, x_test, y_test, y_normalizer = load_data()
...
#     Legendre
    model = torch.load('ptmodels/NS_v_1e-3_N1000_T50_alpha_12_c_4_k_3_nCZ_4_L_0_3CNN_BN_Leg_epoch_500.pt')
    model.to(device)

    l2_test = test(model, test_loader, device, lossFn=myloss, post_proc=y_normalizer.decode)
    print(f'test relative L2 error for N=1000, T=50, nu=1e-3 with Legendre = {l2_test}')
    
#     Chebyshev

    model = torch.load('ptmodels/NS_v_1e-3_N1000_T50_alpha_12_c_4_k_3_nCZ_4_L_0_3CNN_BN_Chb_epoch_500.pt')
    model.to(device)
    
    l2_test = test(model, test_loader, device, lossFn=myloss, post_proc=y_normalizer.decode)
    print(f'test relative L2 error for N=1000, T=50, nu=1e-3 with Chebyshev = {l2_test}')
